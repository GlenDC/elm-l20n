  ...
  function bootstrap() {
    ...
    if (scripts.length) {
     if (scripts[0].hasAttribute('src')) {
        ctx.linkResource(scripts[0].src);
      } else {
        ctx.addResource(scripts[0].textContent);
      }

      var language = element.scripts[0].getAttribute("language")
      if(language) {
        ctx.requestLocales(language);
      } else {
        ctx.requestLocales();
      }
      return collectNodes();
...
Elm.Native.L20n.make = function(localRuntime){
  ...
  function getElementOrCreate(node, element) {
    var nodeList = node.getElementsByTagName(element);
    if(nodeList && nodeList.length != 0) {
      return nodeList[0];
    } else {
      var newNode = node.createElement(element);
  ...
  var view = function(config) {
    var head = getElementOrCreate(document, "head");
    ...
    var l20nScript = document.createElement("script");
    l20nScript.setAttribute("type", "application/l10n-data+json");
    l20nScript.innerHTML = '{"build":"prod","mode":"multi"}';
    head.appendChild(l20nScript);

    var manifest = getJSON(config.url);
    if(manifest) {
      return Utils.Tuple2(manifest.default_locale, List.fromArray(manifest.locales));
    } else {
      return Utils.Tuple2("", List.fromArray([]));
    }
  };

  var title = function(id, args) {
    var head = getElementOrCreate(document, "head");
    var title = getElementOrCreate(head, "title");

    // Set id identifier
    title.setAttribute("data-l10n-id", id);

    // Args is optional, but if it's defined, we'll set it as well.
    if(args) {
